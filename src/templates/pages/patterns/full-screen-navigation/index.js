...
const FullScreenNavigation = () => <PatternLayout>
    ...
    <p class="push-bottom--double">When the full screen menu is open, it shouldn't be possible for the user to access the content beneath via mouse, keyboard or other means.  At the time of writing, the most cross-device compatible way to achieve this is by setting all other elements to <code>display: none !important;visibility: hidden !important;</code> in CSS using the toggle classes provided on the body tag.</p>
...
`}</code></pre>
    ...
    <ul class="list list--tick push-bottom--double">
        ...
        <li class="list-item">Navigation open and close buttons should be appropriately labelled to describe their functionality. If the design requires no visible text, a label should be added as an <pre class="pre--inline">aria-label</pre> attribute on the <pre class="pre--inline">&lt;button&gt;</pre> element</li>
        <li class="list-item">Navigation toggle buttons should be no less than 44px x 44px in size (unless any of the allowed <a href="https://www.w3.org/TR/WCAG22/#target-size-enhanced">WCAG exceptions apply</a>)</li>
        ...
        <li class="list-item">Navigation links should be visible, available for keyboard access and read by screen readers when the menu is opened</li>
    ...
    <ul class="list list--tick push-bottom--double">
        ...
        <li class="list-item">The navigation close button should receive keyboard focus in a <em>logical order</em>. The design may require it to be positioned top right (for example) but the <pre class="pre--inline">&lt;button&gt;</pre> can be the last HTML element in the navigation, so long as tabbing behaves consistently</li>
